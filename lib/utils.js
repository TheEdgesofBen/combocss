...
function getBreakpointSortValue(size) {
    if (typeof size !== "string") return Number.POSITIVE_INFINITY;

    let match = size.trim().match(/^(\d*\.?\d+)(px|rem|em)$/);
...
function getBreakpointOrder(breakpoints = {}) {
    return Object.keys(breakpoints)
        .map((name, index) => ({ name, index, value: getBreakpointSortValue(breakpoints[name]) }))
        .sort((a, b) => a.value - b.value || a.index - b.index);
}

function getBreakpointRankMap(breakpoints = {}) {
    let rankMap = new Map();

    getBreakpointOrder(breakpoints).forEach((breakpoint, rank) => {
        rankMap.set(breakpoint.name, rank);
    });
...
function addCustomRuleToMap(rule, customMap, layer, atRules = []) {
    let customClassName = rule.selector;
    let customClassMap = {
        combo: "",
        shortcut: "",
        props: [],
    ...
    if (customClassName && (customClassMap.combo || customClassMap.shortcut)) {
        customClassMap.selector = customClassName;
        customClassMap.key = customClassName.slice(1);

        if (layer && customClassMap.combo) {
            customClassMap.layer = layer;
        }

        if (!customMap[customClassMap.key]) {
            customMap[customClassMap.key] = {
                ...customClassMap,
                variants: [customClassMap],
            };
        } else {
            let variants = customMap[customClassMap.key].variants || [customMap[customClassMap.key]];
            variants.push(customClassMap);

            if (!customMap[customClassMap.key].atRules?.length && customClassMap.atRules?.length) {
                customMap[customClassMap.key].variants = variants;
            } else {
                customMap[customClassMap.key] = {
                    ...customClassMap,
                    variants,
...
function walkCustomNodes(nodes = [], customMap, layer, atRules = []) {
    nodes.forEach((node) => {
        if (node.type == "rule") {
            addCustomRuleToMap(node, customMap, layer, atRules);
        } else if (node.type == "atrule" && node.nodes) {
            walkCustomNodes(node.nodes, customMap, layer, [...atRules, { name: node.name, params: node.params }]);
        }
    });
}
...
function getCustomMap(data, layer) {
    ...
    walkCustomNodes(root.nodes, customMap, layer);
...
export {
    ...
    getResponsiveSize,
    getResponsiveName,
    getBreakpointSortValue,
    getBreakpointOrder,
    getBreakpointRankMap,
    isIgnored,
    camelToDash,
    escapeCssSelector,
    splitStringByCSSVar,
