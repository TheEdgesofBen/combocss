...
function identifyClasses(classes, customMap, breakpoints = {}) {
    ...
    function pushIdentified(cssClass, foundCustom = null, sourceClass = cssClass, customVariant = null, variantIndex = 0) {
        const key = `${cssClass}|${foundCustom || ""}|${variantIndex}`;
        if (identifiedKeys.has(key)) return;

        let identifiedClass = { cssClass, sourceClass, custom: false };
        let customData = customVariant || customMap[foundCustom];

        if (foundCustom) {
            identifiedClass.custom = true;
            identifiedClass.customKey = foundCustom;
            identifiedClass.variantIndex = variantIndex;
            identifiedClass.layer = customData.layer;
            identifiedClass.rawSelector = customData.selector;
            identifiedClass.atRules = customData.atRules;
        }
    ...
    classes.forEach((cssClass) => {
        ...
        if (foundCustom) {
            let variants = customMap[foundCustom].variants || [customMap[foundCustom]];
            variants.forEach((variant, index) => pushIdentified(foundCustom, foundCustom, cssClass, variant, index));
        ...
        Object.keys(customMap).forEach((customClass) => {
            if (customClass !== className && getCustomBaseKey(customClass) === className && customMap[customClass].combo) {
                let variants = customMap[customClass].variants || [customMap[customClass]];
                variants.forEach((variant, index) => pushIdentified(customClass, customClass, cssClass, variant, index));
            }
        });
    });
...
function createClassRules(cssClass, customMap, breakpoints, context = {}) {
    ...
    if ((context.customVariant || customMap[comboClassPart]) && (context.customVariant?.combo || customMap[comboClassPart]?.combo)) {
        selectors = splitClassTokens((context.customVariant || customMap[comboClassPart]).combo).map((selector) => inheritFromCustom(selector, classParts[1], false));
    ...
    if (context.customVariant || customMap[getClassParts(cssClass, breakpoints)[1]]) {
        (context.customVariant || customMap[getClassParts(cssClass, breakpoints)[1]]).props.forEach((nextRule) => rules.push(nextRule));
    }
...
function createCSSClassesData(identifiedClasses, customMap, breakpoints = {}, opts = {}) {
    ...
    identifiedClasses.forEach((identifiedClass) => {
        ...
        if (identifiedClass.rawSelector && identifiedClass.variantIndex) className = `${className}__comboVariant${identifiedClass.variantIndex}`;
        ...
        cssClasses[className].atRules = identifiedClass.atRules;
        ...
        cssClasses[className].rules = createClassRules(identifiedClass.customKey || identifiedClass.cssClass, customMap, breakpoints, {
            ...
            strict: opts.strict,
            stack: [],
            customVariant: identifiedClass.custom ? customMap[identifiedClass.customKey]?.variants?.[identifiedClass.variantIndex] : null,
        });
