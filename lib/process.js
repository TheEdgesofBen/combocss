...
function identifyCustomClasses(classes, opts, customClassesMap) {
    ...
    classes.forEach((cssClass) => {
        ...
        if (foundCustom) {
            ...
            identifiedClass.order = foundCustom.order;
        }
...
function createClasses(identifiedClasses, opts, customClassesMap) {
    ...
    identifiedClasses.forEach((identifiedClass, identifiedClassIndex) => {
        if (identifiedClass.order || identifiedClass.order === 0) {
            if (!identifiedClassesByOrder[identifiedClass.order]) {
                identifiedClassesByOrder[identifiedClass.order] = [];
                identifiedClassesByOrder[identifiedClass.order].push(identifiedClass);
            }
        } else {
            if (!identifiedClassesByOrder["unordered"]) {
                identifiedClassesByOrder["unordered"] = [];
                identifiedClassesByOrder["unordered"].push(identifiedClass);
            }
        }
    });
