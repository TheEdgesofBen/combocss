...
const { isResponsive, getResponsiveSize, findLayerIndex, findNode, isIgnored, camelToDash, escapeCssSelector, currentFileClasses } = require("./utils");
...
function getShortcutClassPart(primaryClassPart) {
    ...
    /*
    if (shortcutClassPart.includes("mt")) {
        console.log("getShortcutClassPart", startingSpecialChars);

        console.log(classParts);

        console.log(shortcutClassPart);
    }
    */
...
function identifyCustomClasses(classes, opts, customClassesMap) {
    ...
    classes.forEach((cssClass) => {
        ...
        if (foundCustom) {
            ...
            identifiedClass.order = customClassesMap[foundCustom].order;
...
function inheritFromCustom(className, primaryCustomClassPart, isShortcut = false) {
    ...
    */
    ...
    if (isShortcut) {
        className = className + "-" + primaryCustomClassPart.split(/\-(?![^\(]*\))/g)[primaryCustomClassPart.split(/\-(?![^\(]*\))/g).length - 1];
    }
...
function createClasses(identifiedClasses, opts, customClassesMap) {
    ...
    let identifiedClassesByOrder = {};
    let identifiedClassesSorted = [];

    identifiedClasses.forEach((identifiedClass) => {
        let order = "unordered";

        if (identifiedClass.order || identifiedClass.order === 0) {
            order = identifiedClass.order;
        }

        if (!identifiedClassesByOrder[order]) {
            identifiedClassesByOrder[order] = [];
        }

        identifiedClassesByOrder[order].push(identifiedClass);
    });

    for (let i = 0; i < Object.keys(identifiedClassesByOrder).length; i++) {
        let order = i;

        if (i == Object.keys(identifiedClassesByOrder).length - 1) {
            order = "unordered";
        }

        identifiedClassesSorted.push(...identifiedClassesByOrder[order]);
    }
