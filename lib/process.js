...
function process(opts) {
    ...
    Object.keys(rawFilesClasses).forEach((fileName) => {
        identifiedFilesClasses[fileName] = identifyCustomClasses(Array.from(rawFilesClasses[fileName]), opts, customMap);
    });

    Object.keys(identifiedFilesClasses).forEach((fileName) => {
        newOutputData[fileName] = createOutputData(identifiedFilesClasses[fileName], opts, customMap);
    });
...
function updateOutputFile(newOutputData, opts, layersOrder) {
    ...
    Object.keys(newOutputData).forEach((fileName) => {
        let fileClasses = newOutputData[fileName];
    ...
    Object.keys(classes).forEach((type) => {
        ...
        Object.keys(classes[type]).forEach((selector) => {
            let newNode = null;
            let oldNode = null;
            let layer = null;

            if (classes[type][selector].layer) {
                layer = classes[type][selector].layer.replace(/[^\w-:]/g, "\\$&");
            }

            newNode = createNode(selector, classes[type][selector], opts);
    ...
    Object.keys(classesMap).forEach((selector) => {
        classesMap[selector].forEach((fileName, fileNameIndex) => {
            if (newOutputData[fileName] && !newOutputData[fileName][selector]) {
...
function createOutputData(identifiedClasses, opts, customMap) {
    let outputData = {};
    ...
    identifiedClasses.forEach((identifiedClass, identifiedClassIndex) => {
        ...
        outputData[className] = { rules: [], custom: false };

        if (identifiedClass.custom) {
            outputData[className].custom = true;
        }

        outputData[className].layer = identifiedClass.layer;

        outputData[className].rules = createClassRules(identifiedClass.cssClass, opts, customMap);

        if (outputData[className].rules.length == 0) delete outputData[className];
    });
