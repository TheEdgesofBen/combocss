    ...
    classes.forEach((cssClass) => {
        ...
        if (!foundCustom) {
            foundCustom = Object.keys(customClassesMap).find((customClass) => {
                return getShortcutClassPart(getClassParts(cssClass, opts)[1], customClassesMap) == customClass;
            });
        }
...
function createClassRules(cssClass, opts, customClassesMap) {
    ...
    let comboClassPart = getComboClassPart(classParts[1]);
    ...
    selectors.forEach((selector) => {
        ...
        shortcutClassPart = getShortcutClassPart(classParts[1], customClassesMap);
        ...
        if (customClassesMap[comboClassPart]?.combo || customClassesMap[shortcutClassPart]?.shortcut) {
            ...
            declarationParts = declarationParts.map((declarationPart) => {
                if (declarationParts[1]?.match(/(\(.*?\))/)) {
                    return declarationPart;
                }
                return camelToDash(declarationPart);
            });
