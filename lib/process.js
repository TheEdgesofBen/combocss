...
function updateOutputFile(filesClasses, opts) {
    let filesPaths = fg.sync(opts.output);
    let outFileData = fs.readFileSync(filesPaths[0], "utf8");
    let root = postcss.parse(outFileData);
...
function getCustomClassesMap(opts) {
    let customClassesMap = {};
    let filesPaths = fg.sync(opts.custom?.file || []);
    let filesContent = [];
    ...
    filesContent.forEach((fileContent, fileContentIndex) => {
        ...
        root.nodes.forEach((rule) => {
            ...
            Object.keys(opts.custom?.classes || {}).forEach((customClassName) => {
                if (!customClassesMap[customClassName]) {
                    customClassesMap[customClassName] = {
                        combo: opts.custom.classes[customClassName],
                        props: [],
                    };
                }
            });

            Object.keys(opts.custom?.shortcut || {}).forEach((customClassName) => {
                if (!customClassesMap[customClassName]) {
                    customClassesMap[customClassName] = {
                        shortcut: opts.custom.shortcut[customClassName],
                        props: [],
                    };
                }
            });
            ...
            classValues.forEach((classValue, classValueIndex) => {
                ...
                if ((classValue.match(/{([\s\S]*?)}/g) || []).length > 0 && classValue.indexOf("{") == 0 && classValue.indexOf("}") == classValue.length - 1) {
                    ...
                    classValue.split(",").forEach((classObjProp) => {
                        ...
                        classObjPropKey.match(/[^()\s]+(\(.*?\))?/g).forEach((classObjPropKeyPart) => classes.add(classObjPropKeyPart));
                    });
                } else {
                    if ((classValue.match(/'([\s\S]*?)'/g) || []).length == 0 && classValue.indexOf("'") != 0) {
...
function getClassProp(rawClassProp, opts) {
    ...
    /*
    let shortCutProp = Object.keys(opts.custom.shortcut).find((shortcut) => rawClassProp == shortcut);

    if (shortCutProp) classProp = camelToDash(opts.custom.shortcut[shortCutProp]);
    */
