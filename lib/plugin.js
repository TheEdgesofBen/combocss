...
import { init as utilsInit, isResponsive, currentFileClasses, getCustomMapByGlob, getLayersOrder } from "./utils.js";
import {
    init as postcssInit,
    stringCSSToPostCSS,
    postCSSToStringCSS,
...
let fs, fg, chokidar, fileURLToPath, dirname;
let __dirname = null;
...
async function init(opts, mode = "plugin") {
    if (typeof window !== "object") {
        let {
            ...
            dirname: _dirname,
        } = await getModules("fs", "fg", "chokidar", "fileURLToPath", "dirname");
    ...
    if (mode == "plugin") {
        ...
        const watcher = chokidar.watch(config.input, { useFsEvents: false });
