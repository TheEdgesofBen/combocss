...
import { isResponsive, getResponsiveSize, getResponsiveName, getBreakpointRankMap } from "./utils.js";
...
async function init() {
    if (typeof window !== "object") {
        let { stylelint: _stylelint } = await getModules("stylelint");
...
function createNode(selector, selctorClass, breakpoints) {
    let atRule = null;
    let newRule = null;
    let className = selctorClass.rawSelector || "." + selector;
    let node = null;
    ...
    if (selctorClass.atRules?.length) {
        [...selctorClass.atRules].reverse().forEach((rule) => {
            let wrapper = new AtRule({ name: rule.name, params: rule.params });
            wrapper.append(node);
            node = wrapper;
        });
    }
...
function findNode(node, layer, root) {
    ...
    if (!root?.nodes) return foundNode;
...
function sortSelectors(selectors, type, breakpoints = {}) {
    if (type != "responsive") return selectors;

    let breakpointRankMap = getBreakpointRankMap(breakpoints);

    return selectors
        .map((selector, index) => ({ selector, index }))
        .sort((a, b) => {
            let aBreakpoint = getResponsiveName(a.selector, breakpoints);
            let bBreakpoint = getResponsiveName(b.selector, breakpoints);
            let aRank = breakpointRankMap.get(aBreakpoint) ?? Number.POSITIVE_INFINITY;
            let bRank = breakpointRankMap.get(bBreakpoint) ?? Number.POSITIVE_INFINITY;
...
function getLastTopLevelNonResponsiveNode(root) {
    let lastNoneResponsiveNode = null;

    root.nodes.forEach((node) => {
        if (node.type == "rule") lastNoneResponsiveNode = node;
    });

    return lastNoneResponsiveNode;
}

// Appends combo css node to root and removes old node with same selector and @media at rule inside root
function appendClassNodesToRoot(classes, root, type, breakpoints, layersMap) {
    let lastNoneResponsiveNode = getLastTopLevelNonResponsiveNode(root);

    sortSelectors(Object.keys(classes), type, breakpoints).forEach((selector) => {
        let newNode = null;
        let oldNode = null;
        let layer = null;
        ...
        if (!layer) {
            if (type == "responsive") {
                if (lastNoneResponsiveNode) {
                    ...
                    lastNoneResponsiveNode = newNode;
...
async function enrichCSSWithPlugins(css, plugins = ["autoprefixer"]) {
    let enrichedCSS = css;

    if (plugins.includes("stylelint")) {
        try {
            let result = await stylelint.lint({ code: css, configBasedir: process.cwd() });

            if (result.errored) return false;
        } catch (error) {
            console.warn(`ComboCSS stylelint skipped: ${error.message}`);
        }
    }

    if (plugins.includes("autoprefixer")) {
        try {
            enrichedCSS = await postcss([autoprefixer]).process(css, { from: undefined }).then((result) => result.css);
        } catch (error) {
            console.warn(`ComboCSS autoprefixer skipped: ${error.message}`);
            return false;
        }
    }
